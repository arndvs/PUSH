...
stateDiagram-v2
    ...
    FOLDED : FOLDED — STATE FOUR\nV-shape. Car configuration only.\nHum continues through upholstery.

    FLAT --> RAISED : PUSH button pressed
    RAISED --> VERTICAL : back panel reaches vertical
    VERTICAL --> FOLDED : car entry\n(HIDDEN BY EDIT)
    FOLDED --> VERTICAL : parking lot arrival\n(HIDDEN BY EDIT)
    VERTICAL --> FLAT : arrives home, folds flat
```
...
sequenceDiagram
    ...
    Note over Dave,Bed: PUSH pressed — both begin their morning

    Dave->>Dave: Wakes up (chaotic)
    Bed->>Bed: Rises to vertical (dignified)

    Dave->>Dave: Bathroom — brushing teeth (frantic)
    Bed->>Bed: Sheet tensioning begins (unhurried, 1/3 through)

    Dave->>Dave: Shower (scrambling behind curtain)
    Bed->>Bed: Sheet tensioning continues
    Bed->>Bed: Pillow node begins

    Dave->>Dave: Closet — emerges wrong
    Bed->>Bed: Sheet nearly taut. Pillow fuller but not done.
    Bed->>Dave: Nudges him back in

    Dave->>Dave: Closet — emerges correct
    Bed->>Bed: Final sheet wrinkles smoothing

    Dave->>Dave: Hallway — fighting tie knot
    Bed->>Bed: Pillow node completes. One slow exhale. Settles.

    Note over Dave,Bed: PAYOFF CUT
    Note over Bed: Pillow — plumped, slightly askew. Done.
    Note over Dave: Tie knot — sideways. "Good enough."

    Note over Dave,Bed: Dave never notices. The bed doesn't mention it.
```
