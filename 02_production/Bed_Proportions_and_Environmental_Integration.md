...
**The solution is not scale. The solution is confidence.**
...
## THE BED'S SCALE CONTRACT

The bed has one consistent rule across every environment:

> **The bed always feels like it belongs exactly where it is.**

How that's achieved varies by location — sometimes through environment design (wider hallways, taller ceilings), sometimes through editing (cutting before an impossible transition), sometimes through camera choice (wide or close, never medium), and sometimes through the characters accepting it so completely that the audience does too.

The bed never shrinks or grows. It's the same object in every environment. What changes is the toolkit used to make each environment accept it. The audience should never be aware of which toolkit is being used at any given moment.
...
**Sound design cue:** A low, steady hum. No brushing or scraping sounds — those would imply the fit is tight and call attention to the geometry. The bed moves through the hallway the way it moves everywhere: with complete indifference to the architecture.
...
4. **The hum through the upholstery.** The bed is present in the car primarily through sound — a low vibration felt through the seat. Dave drives. The bed hums. The camera stays on Dave's face and the road. We trust it's there without being shown the full geometry.

5. **The rearview mirror shot:** When Dave looks in the rearview mirror and we see the bed reflected — this is the one moment we confirm it's there. Keep it a close-up on the mirror. The reflection shows a dark surface, an edge, a corner. Enough to confirm. Not enough to analyze.
...
## PROMPT LANGUAGE FOR AI VIDEO TOOLS
...
## SUMMARY CHECKLIST FOR EVERY SHOT WITH THE BED

Before finalizing any shot:

- [ ] Does the environment have enough clearance for the bed? (If not, widen it in the prompt)
- [ ] Is the bed lit by the same sources as the rest of the scene?
- [ ] Does Dave treat the fit as normal?
- [ ] Is the camera angle wide or close — not medium?
